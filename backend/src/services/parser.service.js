...
export const parseCSV = (buffer, filename) => {
  return new Promise((resolve, reject) => {
    ...
    stream
      ...
      .on('end', () => {
        if (!isValid) {
          reject(new AppError(`Missing required columns in ${filename}: ${missingColumns.join(', ')}`, 400));
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (filename === 'weather.json') {
        ...
        if (missing.length > 0) {
          return reject(new AppError(`Missing required keys in weather.json: ${missing.join(', ')}`, 400));
        }
      }
...
export const validateFileContext = async (file) => {
  ...
  if (originalname.endsWith('.csv')) {
    if (!SCHEMAS[originalname]) {
      throw new AppError(`Unsupported CSV file: ${originalname}. Supported are: crowd.csv, gates.csv, incidents.csv`, 400);
    }
