...
export function SafetyCard({ explanation }) {
  ...
  const getRiskProps = () => {
    ...
    if (riskLevel === 'MEDIUM')
      return { color: 'warning', bg: 'warning.50', icon: <FiAlertOctagon /> };
    ...
    >
      ...
      <Box sx={{ bgcolor: props.bg, p: 2, borderRadius: 3, mb: 3 }}>
        <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
          <Box sx={{ color: `${props.color}.main` }}>{props.icon}</Box>
          <Typography variant="body2" color={`${props.color}.dark`} fontWeight={600}>
            {riskLevel === 'HIGH'
              ? 'Proceed with extreme caution. Follow stadium staff instructions.'
              : 'Current route is assessed as safe based on available telemetry.'}
          </Typography>
        </Stack>
      </Box>
