...
export function LiveFeed({ events }) {
  ...
  return (
    <Paper elevation={0} sx={{ p: 4, borderRadius: 4, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
      <Typography variant="h6" fontWeight={700} gutterBottom>Live Activity Feed</Typography>
          ...
          {events.map((event, idx) => (
            <ListItem key={idx} disableGutters sx={{ alignItems: 'flex-start', mb: 1 }}>
              <ListItemIcon sx={{ minWidth: 36, mt: 0.5 }}>
                {event.type === 'ERROR' ? <FiAlertTriangle color={theme.palette.error.main} /> : 
                 event.type === 'WARNING' ? <FiAlertTriangle color={theme.palette.warning.main} /> : 
