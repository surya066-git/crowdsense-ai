...
import { getGates, getCrowdDensity, getIncidents, getWeather, getMapConfig } from '../services/mapService.js';
...
export default function MapPage() {
  ...
  const drawerContent = (
    <Box sx={{ width: { xs: 280, md: 320 }, height: '100%', borderRight: `1px solid ${theme.palette.divider}`, bgcolor: 'background.paper' }}>
      <SidePanel 
        data={mapData} 
        loading={loading} 
        filters={filters} 
      ...
      <Box sx={{ display: 'flex', height: 'calc(100vh - 64px)', overflow: 'hidden', position: 'relative' }}>
        ...
        {isMobile && (
          <Drawer
            ...
            sx={{ '& .MuiDrawer-paper': { boxSizing: 'border-box', width: 280, top: 64, height: 'calc(100% - 64px)' } }}
          >
            {drawerContent}
          </Drawer>
        )}
        ...
        <Box sx={{ flexGrow: 1, position: 'relative' }}>
          {isMobile && (
            <IconButton
              ...
              sx={{ position: 'absolute', top: 16, left: 16, zIndex: 1000, bgcolor: 'background.paper', boxShadow: 2, '&:hover': { bgcolor: 'grey.100' } }}
            >
              <FiMenu />
            </IconButton>
          )}
          ...
          {error && (
            <Alert severity="error" sx={{ position: 'absolute', top: 16, left: { xs: 60, md: 16 }, right: 16, zIndex: 1000 }}>
              {error}
            </Alert>
          )}
