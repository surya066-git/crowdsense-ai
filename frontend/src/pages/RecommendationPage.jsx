...
export default function RecommendationPage() {
  ...
  const fetchRecommendation = async () => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || err.message || 'Failed to generate recommendation.');
      setStatus('ERROR');
    }
  };
        ...
        <Grid container spacing={3}>
          ...
          <Grid xs={12} md={4}>
            <Stack spacing={3}>
              ...
              <Box sx={{ p: 4, borderRadius: 4, bgcolor: 'grey.50', border: `1px solid ${theme.palette.divider}` }}>
                <Typography variant="subtitle2" fontWeight={700} gutterBottom>Journey Timeline</Typography>
                <Typography variant="body2" color="text.secondary">Generated: {new Date(recommendation.timestamp).toLocaleTimeString()}</Typography>
                <Typography variant="body2" color="text.secondary">Recommendation ID: <br/><span style={{ fontSize: '0.75rem' }}>{recommendation.recommendationId}</span></Typography>
              </Box>
            </Stack>
          </Grid>
        </Grid>
